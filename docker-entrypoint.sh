...
shutdown () {
   sleep 1
   echo "......SIGNAL system shutdown cluster server"
   mysqladmin shutdown -p${MYSQL_ROOT_PASSWORD} -uroot
   echo "......cluster server shutdown complete"
   sleep 3
}
...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
            ...
            #mysql_note "......Starting temporary cluster server"
            #declare -g NEW_CLUSTER
            #NEW_CLUSTER=""
            #if [ -f "/var/lib/mysql/GALERIA" && -n "${INIT_CLUSTER}" ]; then 
            #   NEW_CLUSTER="--wsrep-new-cluster"
            #   mysql_note "......New cluster init"
            #   touch /varlib/mysql/GALERIA
            #fi
            #echo "$@"
			#"$@" \
			#--wsrep_on=${WSREP_ON} \
			...
			#--wsrep_node_name="${WSREP_NODE_NAME}" --wsrep-new-cluster &
			#${NEW_CLUSTER} 
            #mysql_note "......Check cluster size"
            #for i in {30..0}; do
            #    s=$( echo "SHOW GLOBAL STATUS like 'wsrep_cluster_size';" | \
            #    mysql -p${MYSQL_ROOT_PASSWORD} -uroot &> /dev/null| grep "wsrep_cluster_size" | \
            #    awk -e '{ print $2}' )
            #    echo "......clustersize=${s}"
            #    if  ${s} ; then 
            #      break 
            ...
            #done
            #mysql_note "......Stopping temporary server"
			#mysqladmin shutdown -uroot -p${MYSQL_ROOT_PASSWORD} &> /dev/null
			#mysql_note "......Temporary server stopped"			
    ...
    #if [ ! -z "$DATABASE_ALREADY_EXISTS" ]; then
    #    mysql_note "......normal server"
    #else
        mysql_note "......galeria server"
        set -- "$@" \
        --wsrep_on=${WSREP_ON} \
        --wsrep_provider=${WSREP_PROVIDER} \
        --wsrep_cluster_name="${WSREP_CLUSTER_NAME}" \
		...
		if [ ! -z "${NEW_CLUSTER}" ]; then 
		  ...
		  set -- "$@" \
		   --wsrep-new-cluster
          if [  $(awk -e '( /^safe_to_bootstrap/){ print $2 }' /var/lib/mysql/grastate.dat) -eq 0 ] ; 
          then 
            sed -i -e 's/safe_to_bootstrap: 0/safe_to_bootstrap: 1/ig' /var/lib/mysql/grastate.dat	
            mysql_note "......bootstrap modified to 1"; 
          fi
		fi
    #fi	
	exec "$@"
